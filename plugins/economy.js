// plugins/economy.js
// Enhanced Economy & Underworld Plugin for Queen-Kylie-V1 — Weirdos World
...
// BLACKMARKET
cmd({ pattern: "bm", desc: "View underworld items (alias: blackmarket)", category: "rpg", filename: __filename }, async (conn, mek, m, { reply }) => {
...
cmd({ pattern: "underworld", desc: "Buy from black market (alias: deal)", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
...
cmd({ pattern: "invest", desc: "invest status | invest <slot> <amount> <term> | invest claim/withdraw <slot>", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      const id  = await getPlayerId(conn, m.sender);
      const acc = await getAccount(id);
      if (isHospitalized(acc)) return reply(`🚨 Action restricted.`);
...
cmd({ pattern: "claim", desc: "Alias: invest claim <slot>", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
...
// =============================================================================
cmd({ pattern: "wirecrypto", desc: "wirecrypto @player <coin> <amount> — Send crypto to another player", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      const senderId = await getPlayerId(conn, m.sender);
...
// =============================================================================
cmd({ pattern: "paycrypto", desc: "paycrypto @player <coin> <amount> — Pay someone in crypto", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      const senderId = await getPlayerId(conn, m.sender);
      const acc      = await getAccount(senderId);
...
cmd({ pattern: "statement", desc: "View full financial statement — statement [page] | statement @player", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      const senderId   = await getPlayerId(conn, m.sender);
      const CREATOR_ID = '2348084644182';
  ...
  try {
      ...
      if (subCmd === 'invitechild') {
          ...
          if (mySession.openUntil < now()) {
              gangRobSessions.delete(myTargetId);
...
cmd({ pattern: "econdrains", desc: "Check all active drains and toggle status", category: "rpg", filename: __filename }, async (conn, mek, m, { reply }) => {
  try {
      const senderId = await getPlayerId(conn, m.sender);
      const acc      = await getAccount(senderId);
      const total    = (acc.money || 0) + (acc.bank || 0);
...
// =============================================================================
cmd({ pattern: "robshield", desc: "Buy temporary rob immunity for 2 hours", category: "rpg", filename: __filename }, async (conn, mek, m, { reply }) => {
  try {
      const id  = await getPlayerId(conn, m.sender);
      const acc = await getAccount(id);
...
cmd({ pattern: "quickduel", desc: "quickduel @player <amount> — fast in-memory cash duel", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      const id  = await getPlayerId(conn, m.sender);
      const acc = await getAccount(id);
...
const OBBY_TIERS = [
  { name: 'Beginner',   energy: 15, baseReward: iC(500),   speedReq: 0,   failChance: 0.20 },
  { name: 'Normal',     energy: 25, baseReward: iC(1500),  speedReq: 20,  failChance: 0.25 },
  { name: 'Hard',       energy: 40, baseReward: iC(5000),  speedReq: 50,  failChance: 0.35 },
  { name: 'Insane',     energy: 60, baseReward: iC(15000), speedReq: 100, failChance: 0.45 },
  { name: 'Impossible', energy: 80, baseReward: iC(40000), speedReq: 200, failChance: 0.55 },
];
...
cmd({ pattern: "obby", desc: "obby [difficulty] — run an obstacle course for cash", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
      ...
      acc.money   = capBalance((acc.money||0) + net);
      const obbyLoanMsg = tryDeductLoan(acc, net, `Obby (${tier.name})`);
      acc.endurance   = (acc.endurance||0) + 3; // obby = big endurance gains
...
cmd({ pattern: "workingstats", desc: "View your working stats (Manual Labor, Intelligence, Endurance)", category: "rpg", filename: __filename }, async (conn, mek, m, { reply }) => {
  try {
      const id  = await getPlayerId(conn, m.sender);
      const acc = await getAccount(id);
...
// ─────────────────────────────────────────────────────────────────────────────
cmd({ pattern: "pay", desc: "pay @player <amount> — Alias for wire", category: "rpg", filename: __filename }, async (conn, mek, m, { args, reply }) => {
  try {
