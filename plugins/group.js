...
cmd({
  ...
  category: "group",
  filename: __filename,
}, async (conn, mek, m, { from, args, q, reply, isGroup, isAdmins, isOwner, isSudo, participants }) => {
  try {
    if (!isGroup) return reply("🚫 *Groups only!*");
    if (!isAdmins && !isOwner && !isSudo) return reply("⚠️ *You need admin permissions.*");
...
cmd({
  ...
  try {
    if (!isGroup) return reply("🚫 *Groups only!*");
    if (!isAdmins && !isOwner && !isSudo) return reply("⚠️ *Admin only.*");
...
cmd({
  pattern: "tagall",
  alias: ["tag", "mentionall"],
  desc: "Mentions everyone in the group",
  category: "group",
  filename: __filename,
}, async (conn, mek, m, { from, reply, isGroup, groupMetadata, groupName, sender, q, isAdmins, isOwner, isSudo }) => {
  try {
    if (!isGroup || (!isAdmins && !isOwner && !isSudo)) return reply("🚫 *Admins only.*");
...
cmd({
  pattern: "hidetag",
  desc: "Tags everyone silently without showing list",
  category: "group",
  filename: __filename,
  ...
  try {
    ...
    const messageText = q || (m.quoted ? (m.quoted.text || m.quoted.caption || "👀") : "👀");
...
cmd({
  pattern: "tag",
  alias: ["mention"],
  desc: "Tag everyone silently. If message has a group link, sends with the group DP.",
  category: "group",
  filename: __filename,
}, async (conn, mek, m, { from, q, isGroup, groupMetadata, isAdmins, isOwner, isSudo }) => {
  try {
    ...
    if (inviteMatch) {
      ...
      try {
        ...
        const groupSubject = groupInfo.subject || 'WhatsApp Group';
        const memberCount = (groupInfo.participants || []).length;

        // Try to fetch the group DP
      ...
      // If group info/DP fetch failed, fall through to plain send
    }
...
cmd({
  ...
  if (parsed.type === 'delay') {
    ...
    existing.openJob = cron.schedule(parsed.value, async () => {
      try {
        await conn.groupSettingUpdate(from, "not_announcement");
        await conn.sendMessage(from, { text: `🔓 *Group is now open!* _(Scheduled)_` });
      } catch {}
...
cmd({
  ...
  if (parsed.type === 'delay') {
    ...
    existing.closeJob = cron.schedule(parsed.value, async () => {
      try {
        await conn.groupSettingUpdate(from, "announcement");
        await conn.sendMessage(from, { text: `🔒 *Group is now closed!* _(Scheduled)_` });
      } catch {}
    }, { scheduled: true, timezone: "Africa/Lagos" });
