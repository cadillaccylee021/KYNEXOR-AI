...
const nerdamer = require("nerdamer/all"); // make sure to install: npm i nerdamer
...
cmd({
  pattern: "tourl",
  desc: "Create paste and return link",
  category: "tools",
  filename: __filename
}, async (conn, mek, m, { args, reply }) => {
  try {
...
// getpp - profile picture (best-effort)
cmd({
  pattern: "getpp",
  desc: "Get profile picture of a jid/mention",
  category: "tools",
  filename: __filename
}, async (conn, mek, m, { args, from, reply }) => {
  try {
    ...
    if (!jid) {
      // if message mentions exist, try to use them
      jid = (m.mentionedJid && m.mentionedJid.length) ? m.mentionedJid[0] : (mek.key && mek.key.participant) ? mek.key.participant : null;
    }
    if (!jid) return reply("Usage: " + PREFIX + "getpp @number or provide a jid");
    // normalize if mention like @123 -> append domain
    if (!jid.includes("@")) jid = jid + "@s.whatsapp.net";
    // try baileys profilePictureUrl
    try {
      const ppUrl = await conn.profilePictureUrl(jid, "image");
      if (ppUrl) {
        await conn.sendMessage(from, { image: { url: ppUrl }, caption: `Profile picture of ${jid}` }, { quoted: mek });
        return;
      }
    } catch (e) {
      // try fallback via avatar fetch
    }
    return reply("Profile picture not available or private.");
  } catch (e) {
    return reply("Error getting profile picture: " + (e.message || e));
  }
});
...
cmd({
  pattern: "settimezone",
  desc: "Set timezone (IANA) e.g. Europe/Athens",
  category: "settings",
  filename: __filename
}, async (conn, mek, m, { args, isOwner, reply }) => {
  try {
    if (!isOwner) return reply("Only bot owner can change settings.");
    const tz = args && args[0] ? args[0] : "";
    if (!tz) return reply("Usage: " + PREFIX + "settimezone Europe/Athens");
    runtimeSettings.timezone = tz;
    saveSettings(runtimeSettings);
    return reply("Timezone saved: " + tz);
  } catch (e) {
    return reply("Error saving timezone");
  }
});
...
cmd({
  ...
  try {
    if (!isOwner) return reply("Only bot owner can change mode.");
    const mdo = args && args[0] ? args[0].toLowerCase() : "";
    if (!["private", "public"].includes(mdo)) return reply("Usage: " + PREFIX + "mode private|public");
    runtimeSettings.mode = mdo;
    saveSettings(runtimeSettings);
    return reply("Mode saved: " + mdo);
  } catch (e) {
    return reply("Error saving mode");
  }
});
...
cmd({
  ...
  try {
    ...
    return reply("Error reading settings");
  }
});*/

// fallback catch-all for unsupported answers? Not needed here.

module.exports = {}; // plugin registers via cmd