...
const aiImgCmd = (pattern, type, desc) => {
  cmd({ pattern, desc, category: 'tools', filename: __filename },
  async (conn, mek, m, { from, reply }) => {
    try {
      ...
      // Use m.quoted.getbuff (queen kylie's native way) or fall back to download
      let buf;
      if (m.quoted) {
...
cmd({
  ...
  category: 'group',
  filename: __filename
}, async (conn, mek, m, { from, q, isGroup, isAdmins, isBotAdmins, isOwner, isSudo, reply }) => {
  try {
    ...
    await conn.sendMessage(from, { disappearingMessagesInChat: seconds });
...
cmd({
  ...
  category: 'group',
  filename: __filename
}, async (conn, mek, m, { from, args, isGroup, isAdmins, isBotAdmins, isOwner, isSudo, reply }) => {
  try {
